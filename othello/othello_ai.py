...
def ai_evaluate(board):
    ...
    score += (predict_board.black - predict_board.white) * 10 * predict_board.player
    return score

# 改良版評価関数: 盤面の角の安定性を加味

def ai_improved_evaluate(board):
    predict_board = ob.othello_board()
    predict_board.board = board.board
    predict_board.player_board = board.player_board
    predict_board.opponent_board = board.opponent_board
    ...
    score = ai_evaluate(board)

    # 角の安定性評価
    corners = [(0, 0), (0, 7), (7, 0), (7, 7)]
    for (x, y) in corners:
        if predict_board.board[x][y] == predict_board.player:
            score += 25  # 自分の駒
        elif predict_board.board[x][y] == -predict_board.player:
            score -= 25  # 相手の駒
